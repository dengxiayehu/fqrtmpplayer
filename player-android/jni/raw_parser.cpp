...
{
    if (idx >= m_nalus.size()) {
        E("idx %d out of nalu vector", idx);
        return NULL;
    }
...
int AudioRawParser::process(byte *dat, uint32_t len)
{
    ...
    m_raw_len = len - 7;
    byte *buf = (byte *) m_mem_pool.alloc(m_raw_len);
...
#ifdef XDEBUG
    I("profile=%u, sample_rate_idx=%u, channel=%u",
