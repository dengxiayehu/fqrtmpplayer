...
bool File::open(const std::string &path, const char *mode)
{
    ...
    if (path.empty() || !mode) {
        E("Empty path or NULL mode passed (%s:%s)", path.c_str(), mode);
        return false;
    }

    m_fp = fopen(path.c_str(), mode);
    if (!m_fp) {
        E("fopen file \"%s\" failed: %s",
                path.c_str(), ERRNOMSG);
        return false;
    }
...
bool File::seek_ahead(off_t cnt) const
{
    ...
    if (cursor() + cnt > size()) {
        E("Dest pos exceeds file end");
        return false;
    }

    if (fseeko(m_fp, cnt, SEEK_CUR) < 0) {
        E("Unable to seek ahead %ld bytes: %s",
                cnt, ERRNOMSG);
        return false;
    }
...
bool File::seek_behind(off_t cnt) const
{
    ...
    if (cursor() < cnt) {
        E("Dest pos rewind exceeds begin");
        return false;
    }

    if (fseeko(m_fp, (-1) * cnt, SEEK_CUR) < 0) {
        E("Unable to seek behind %ld bytes: %s",
                cnt, ERRNOMSG);
        return false;
    }
...
bool File::seek_to(off_t pos) const
{
    if (fseeko(m_fp, pos, SEEK_SET) < 0) {
        E("Unable to seek position %ld", pos);
        return false;
    }
...
bool File::seek_begin() const
{
    if (fseeko(m_fp, 0, SEEK_SET) < 0) {
        E("Seek to the beginning failed: %s", ERRNOMSG);
        return false;
    }
...
bool File::seek_end() const
{
    if (fseeko(m_fp, 0, SEEK_END) < 0) {
        E("Seek to the end of file failed: %s", ERRNOMSG);
        return false;
    }
...
bool File::read_buffer(uint8_t *buf, size_t sz) const
{
    if (fread(buf, sz, 1, m_fp) < 1) {
        if (ferror(m_fp)) {
            E("Unable to read %lu bytes to buffer: %s",
                    sz, ERRNOMSG);
            return false;
        }

        if (feof(m_fp)) {
            I("Read to end-of-file");
            return false;
        }

        // Neither of them, see whether |sz| is zero
        if (sz == 0) {
            // W("Try to read 0 bytes from file");
            return true;
        }

        W("fread returned 0 unexpectedly");
        return false;
    }
...
bool File::write_buffer(const uint8_t *buffer, long len) const
{
    ...
    if (fwrite(buffer, len, 1, m_fp) < 1) {
        if (ferror(m_fp)) {
            E("Unable to write %ld bytes to file: %s",
                    len, ERRNOMSG);
...
std::string File::read_content(const std::string &path)
{
    ...
    if (!buf) {
        E("malloc for file(\"%s\")'s content failed: %s",
                STR(path), ERRNOMSG);
        return "";
    }
...
int File::flush_content(const std::string &path, const uint8_t *buf, int buf_size, const char *mode)
{
    ...
    if (!file.open(STR(path), mode)) {
        E("Open file(\"%s\") failed", STR(path));
        return -1;
    }
    if (!file.write_buffer(buf, buf_size)) {
        E("Write content to file(\"%s\") failed", STR(path));
        return -1;
    }
    file.close();
    return 0;
