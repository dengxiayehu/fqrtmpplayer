...
void print_asc(const AudioSpecificConfig &asc)
{
    ...
    if (parse_asc(asc, profile, sample_rate_idx, channel) < 0) {
        E("Parse AudioSpecificConfig failed");
        return;
    }
...
int str_to_audioprof(const char *str)
{
    ...
    E("Not supported aac profile %s", str);
    return -1;
}
...
const char *audioprof_to_str(int aprof)
{
    ...
    E("Not support aac profile %d", aprof);
    return NULL;
}
...
int str_to_samplerate_idx(const char *str)
{
    ...
    for (idx=0; samplerate[idx]!=atoi(str); ++idx) {
        if (samplerate[idx] == 0) {
            E("Invalid audio sample rate %s", str);
            return -1;
        }
    }
    return idx;;
...
int h264_decode_sps(GetBitContext *gb, SPS *sps)
{
    ...
    if (sps_id >= MAX_SPS_COUNT) {
        E("sps_id %u out of range", sps_id);
        return -1;
    }
    ...
    if (sps->profile_idc == 100 ||  // High profile
        ...
        if (sps->chroma_format_idc > 3) {
            E("SPS: chroma_format_idc %u not supported",
                    sps->chroma_format_idc);
            goto fail;
        } else if (sps->chroma_format_idc == 3) {
            sps->residual_color_transform_flag = get_bits1(gb);
            if (sps->residual_color_transform_flag) {
                E("sps: separate color planes are not supported");
                goto fail;
            }
        }
        ...
        if (sps->bit_depth_chroma != sps->bit_depth_luma) {
            E("SPS: Different chroma and luma bit depth");
            goto fail;
        }
        if (sps->bit_depth_luma > 14 || sps->bit_depth_chroma > 14) {
            E("SPS: illegal bit depth value (%d, %d)\n",
    ...
    if (log2_max_frame_num_minus4 < MIN_LOG2_MAX_FRAME_NUM - 4 ||
        ...
        E("SPS: log2_max_frame_num_minus4 out of range (0-12): %d",
    ...
    if (sps->poc_type == 0) {
        ...
        if (t > 12) {
            E("SPS: log2_max_poc_lsb (%d) is out of range", t);
            goto fail;
        }
        ...
        if ((unsigned) sps->poc_cycle_length >=
            ...
            E("SPS: poc_cycle_length overflow %d", sps->poc_cycle_length);
            goto fail;
        }
        ...
        E("SPS: illegal POC type %d", sps->poc_type);
        goto fail;
    }
    ...
    if (sps->ref_frame_count > H264_MAX_PICTURE_COUNT - 2 ||
        ...
        E("SPS: too many reference frames %d\n", sps->ref_frame_count);
        goto fail;
    }
    ...
    if ((unsigned)sps->mb_width  >= INT_MAX / 16 ||
        ...
        E("SPS: mb_width/height overflow");
        goto fail;
    }
